...
Since the topic came up, here's a statement to find the 20 most balanced gender-neutral names (out of the names with more than 1000 babies in a year).
...
GenderNeutral <-
  ...
  mutate( ratio=pmin(M/F, F/M) ) %>%
  filter( row_number(desc(ratio)) <= 20)
```
