...
September 2014 \hspace{1in}
{\sf \bfseries \Large Enough R for Data \& Computing Fundamentals}  \hspace{1in}
Daniel Kaplan
\end{center}

\def\opt#1{#1}
\def\squeeze{\vspace*{-4ex}}
A couple dozen functions suffice to carry out your work in Data \& Computing Fundaments.  
...
\asection{Data Tables} are organized into cases and variables.  Variables are either quantitative or categorical: numbers or words.
...
$\bullet$ Second example table: \texttt{Sports}
<<eval=TRUE,echo=FALSE>>=
Sports <- data.frame(
...
\parbox{5.5in}{\asection{Data Verbs} take a data table as input and return as output a modified table.

\smallskip

\hspace*{-.2in}\begin{tabular}{llrl}
...
Cases matched by *all* variables that appear in both tables, just \texttt{name} in the example.

\smallskip

$\bullet$\Keep Keep all cases that have a match:\\
...
@
\end{small}
Note: output has {\em both} of Alma's sports.

\smallskip

$\bullet$ Keep all cases from left table:\\
\texttt{DT \%>\% merge( Sports, all.x=TRUE )}\\
Use \texttt{all=TRUE} to keep all cases from both tables.
...
\asection{Summary Functions} take a variable as input and return a single number.
...
\medskip
\asection{Transformation Functions} are used with \texttt{mutate()}.  They take one or more variables as input and return a variable (with the same number of cases).
...
$\bullet$ Identify group by color\\
\texttt{group=sex}
\smallskip

$\bullet$ \texttt{aes()} to map variables to graphical attributes (aesthetics).

Example:
<<results='hold',echo=1:4,eval=TRUE>>=
DT %>%
...
In \texttt{mUSMap()}, the `key=` argument identifies the variable naming geographic entity and `fill=` specifies the quantity to be plotted.
<<eval=TRUE,message=FALSE,warning=FALSE>>=
plot(mUSMap(data=StateData, 
            key="State",fill="age") )
@
Similarly for \texttt{mWorldMap()}.
