...
```
<p class="caption"> A very small subset of a data table showing how many babies were given each name in each year.</p>
</div>
...
<p class="caption"> The Minneapolis election data in tidy form.</p>
</div>
...
<div class="displayAsFigure">
```{r echo=FALSE}
ggplot(data=neat,aes(x=Precinct,y=Total.Turnout))+geom_point() + ylim(0,.55) + ylab("Voter Turnout (%)")
```
<p class="caption">A graphical depiction of voter turnout in the different wards.</p>
</div>
<div style="clear:both;"></div> <!-- break between caption and text -->
...
```
<p class="caption">An excerpt of runners' performance over the years in a 10-mile race.</p>
</div>
...
For data provided by R packages, a codebook is available via the `help()` function, e.g., for the `BabyNames` data in the `DCF` package, try:
```{r eval=FALSE}
help( BabyNames )
```
