...
What does this graph tell you about pregnancy?
    ...
    ggplot(data=NHANES, aes(x=pregnant, y=age)) + 
        geom_violin()  + theme(legend.position="none") + labs(title="") 

    ggplot(data=NHANES, aes(x=pregnant, y=age)) + 
        geom_violin()  + aes(colour=sex) + 
        facet_wrap(~smoker, ncol=4) + theme(legend.position="none") +
...
Make a graph to indicate whether smokers are systematically lower in weight than non-smokers.
    ...
    ggplot(data=NHANES, aes(x=smoker, y=BMI)) + 
