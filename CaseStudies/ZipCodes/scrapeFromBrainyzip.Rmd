...
fixZIPS <- function(tmp,zips=myzips$zip_code) {
  A <- data.frame(tmp$A)
  A$ZIP <- fixZipZeros(as.character(zips))
...
#  res5[1,] <- unlist(lapply(zipInfo$five[nms5],as.character))
  ...
  for (bunch in 1:length(starts)) {
    ...
    zipVector <- fixZipZeros(as.character(myzips$zip_code[inds]))
    ...
    tmp <- fixZIPS(tmp,as.character(zipVector))
...
In a first scrape, the zip codes were left as NA.  Fix that and consolidate the files into one big file.

```{r}
consolidateZips <- function(bunchSize) {
  myzips <- read.csv("/Users/kaplan/KaplanFiles/DCF-2014/CaseStudies/ZipCodes/zip_codes_states.csv")
  geo <- NULL
  demo <- NULL
  starts <- seq(1,nrow(myzips),by=bunchSize)
  for (bunch in 1:length(starts)) {
    ...
    inds <- inds[inds<=nrow(myzips)]
    zipVector <- fixZipZeros(as.character(myzips$zip_code[inds]))
    load(file=paste('geo-',bunch,'.rda',sep=''))
    load(file=paste('demo-',bunch,'.rda',sep=''))
    zipGeography$ZIP <- zipVector
    zipDemographics$ZIP <- zipVector
    geo <- rbind(geo, zipGeography)
    demo <- rbind(demo, zipDemographics)
  } 
  return(list(geo=geo,demo=demo))
}
```
