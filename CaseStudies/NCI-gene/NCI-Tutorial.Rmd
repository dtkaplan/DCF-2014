...
Fres<-melt(Fres,variable='Measurements',value.name='Values')
Fres$probe<-with(Fres,reorder(probe,Values))
head(Fres)
#Make the plot
p<-ggplot(data=Fres)+geom_point(aes(y=probe,x=Values,color=Measurements))+scale_x_log10()
...
```{r echo=FALSE,eval=FALSE}  
pmin<-Fres$probe[96:100]    not working properly
d2<-filter(bothall,Probe %in% pmin) 
transd2<-cast(d2,Cell.Line.Name+tissue.of.origin+Name.1~Probe,value='expression')
